"""
EGX Event Detector — scans recent news for high-impact events that should
trigger an immediate sentiment refresh for affected stocks.

Called at the START of run_agents.py before generating predictions.
If high-impact events are detected, a targeted sentiment refresh runs
for affected symbols before the agent pipeline proceeds.

Usage:
    from engines.event_detector import EGXEventDetector
    detector = EGXEventDetector(db_conn)
    triggered = detector.scan_and_refresh()
    # triggered = [('COMI.CA', 'earnings', 'immediate'), ...]
"""
...
class EGXEventDetector:
    ...
    EVENT_PATTERNS = {
        'cbe_decision': {
            'keywords_en': [
                'CBE', 'central bank rate', 'overnight deposit', 'interest rate decision',
                'monetary policy committee', 'MPC meeting',
            ],
            'keywords_ar': [
                'البنك المركزي', 'سعر الفائدة', 'لجنة السياسة النقدية',
                'سعر الإيداع', 'سعر الكوريدور',
            ],
            'affects': 'all',
            'urgency': 'immediate',
        },
        ...
        'regulatory': {
            'keywords_en': [
                'FRA', 'Financial Regulatory Authority', 'EGX suspension',
                'trading halt', 'delisted', 'regulatory action',
            ],
            'keywords_ar': [
                'الرقابة المالية', 'وقف التداول', 'شطب', 'إجراء تنظيمي',
            ],
            'affects': 'mentioned_symbols',
            'urgency': 'immediate',
        },
        'index_rebalance': {
            'keywords_en': [
                'EGX30 rebalance', 'index constituent', 'index review',
                'added to EGX30', 'removed from EGX30',
            ],
            'keywords_ar': [
    ...
    # Known EGX ticker patterns for extraction
    _TICKER_PATTERN = re.compile(r'\b([A-Z]{3,5})\.CA\b')
    ...
    def _extract_symbols(self, text: str) -> list:
        """Extract EGX ticker symbols from text."""
        # Direct ticker mentions (e.g. COMI.CA)
        ...
        try:
            ...
            # EGX_STOCKS is a list of symbol strings
            for sym in EGX_STOCKS:
                base = sym.replace('.CA', '').lower()
