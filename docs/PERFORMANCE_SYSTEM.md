...
The `run_agents.py` cost gate was comparing a **1-day ATR%** to a 5-day hold threshold, killing every EGX bluechip under 2.0% daily ATR. It is now horizon-adjusted:

```
5d_expected_move = ATR_1d × sqrt(5)
HOLD if 5d_expected_move < round_trip_cost (0.5%) + min_net_profit (1.0%)
```

This restores actionable UP/DOWN signals for the typical 1.0–1.8% daily-ATR EGX universe without sacrificing the cost-discipline goal.

---

## Market-aware Freshness (April 5, 2026)

`marketAdjustedAgeHours` in `web-ui/server.js` subtracts Fri/Sat hours from data-source staleness calculations for sources that only refresh on EGX trading days. Prevents the Intelligence Pulse widget from showing false "stale" warnings during the weekend.
