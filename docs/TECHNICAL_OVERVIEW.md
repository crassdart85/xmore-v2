...
-   `engines/event_detector.py`: Scans recent news for high-impact events (SAMA rate decisions, earnings, CMA regulatory actions, TASI/MT30 rebalances, price gaps) and triggers targeted sentiment refreshes before the agent pipeline runs.
...
-   **`engines/macro_data.py`**: `MacroDataProvider` class that caches KSA macro indicators (SAMA rate, USD/SAR, CPI YoY, GDP growth) in the `macro_indicators` DB table. Computes a composite `macro_risk_score` (0–1) fed into `SimulationConfig` for regime-aware simulations.
