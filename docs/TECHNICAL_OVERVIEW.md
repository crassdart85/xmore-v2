...
`run_agents.py` rejects directional signals whose expected 5-day move cannot clear round-trip costs. The gate previously compared **1-day ATR%** against a 2.0% threshold, which killed every EGX bluechip. It now scales the daily ATR by √5 (≈ 2.236) to estimate the 5-day move and compares against `round_trip_cost + min_net_profit = 0.5% + 1.0% = 1.5%`.

### 8.2 Market-aware freshness (`marketAdjustedAgeHours`)

The `/api/intelligence/quality` endpoint in `web-ui/server.js` exposes a market-adjusted age for data sources that only refresh on EGX trading days. Fri (UTC day 5) and Sat (UTC day 6) hours are subtracted from the calendar age, so Thursday's post-market data does not look stale on Sun morning. Sources flagged `marketAware: true`: prices, predictions, consensus, sentiment, fx_rates. Calendar age is also returned for diagnostics.
