...
const _PRO_I18N = {
  en: {
    ...
    tasiTitle: 'TASI — Intraday', tasiBlueChips: 'Tadawul Leaders',
  ...
  ar: {
    ...
    tasiTitle: 'تاسي — خلال اليوم', tasiBlueChips: 'قادة تداول',
...
// ── TASI Proxy Chart (Saudi Aramco 2222.SR — 80% of TASI by weight) ─────────
(function loadTASIChart() {
  const container = document.getElementById('tasiChartWidget');
  if (!container) return;

  // Update panel title to reflect data source
  const titleEl = container.closest('.pro-ksa-chart-panel')?.querySelector('.pro-panel-title');
  if (titleEl) titleEl.textContent = 'Saudi Aramco (2222.SR) — 60 Day';

  const canvas = document.createElement('canvas');
  canvas.id = 'tasiNativeChart';
  canvas.style.cssText = 'width:100%;height:100%;min-height:220px';
  container.appendChild(canvas);

  fetch('/api/prices/history/2222.SR?days=60')
    .then(r => r.json())
    .then(rows => {
      if (!rows || !rows.length) {
        container.innerHTML = '<p style="color:#888;padding:16px;font-size:13px;">No price data available yet.</p>';
        return;
      }
      ...
      const fillColor = isUp ? 'rgba(0,200,83,0.12)' : 'rgba(255,23,68,0.12)';
      ...
      new Chart(canvas, {
        ...
        options: {
          ...
          plugins: { legend: { display: false }, tooltip: {
            callbacks: { label: ctx => `SAR ${ctx.parsed.y.toFixed(2)}` },
          }},
          scales: {
            x: { ticks: { color: '#888', maxTicksLimit: 8, font: { size: 11 } }, grid: { color: 'rgba(255,255,255,0.05)' } },
            y: { ticks: { color: '#888', font: { size: 11 }, callback: v => `${v.toFixed(0)}` }, grid: { color: 'rgba(255,255,255,0.05)' } },
          },
        },
      });
    })
    .catch(() => {
      container.innerHTML = '<p style="color:#888;padding:16px;font-size:13px;">Chart data unavailable.</p>';
    });
})();

// ── Tadawul Leaders (native table from /api/prices) ─────────────────────────
const KSA_LEADERS = [
  { symbol: '2222.SR', name: 'Saudi Aramco' },
  { symbol: '2010.SR', name: 'SABIC' },
  { symbol: '1120.SR', name: 'Al Rajhi' },
  { symbol: '7010.SR', name: 'stc' },
  { symbol: '1150.SR', name: 'Alinma' },
  { symbol: '1180.SR', name: 'SNB' },
  { symbol: '1211.SR', name: 'Maaden' },
  { symbol: '2082.SR', name: 'ACWA Power' },
];

(function loadKSAIndices() {
  const container = document.getElementById('ksaIndicesWidget');
  if (!container) return;
  container.innerHTML = '<div style="color:#888;font-size:12px;padding:12px;">Loading…</div>';

  fetch('/api/prices')
    .then(r => r.json())
    .then(allPrices => {
      const bySymbol = {};
      allPrices.forEach(p => { bySymbol[p.symbol] = p; });

      const rows = KSA_LEADERS.map(({ symbol, name }) => {
        const p = bySymbol[symbol];
        const close = p ? parseFloat(p.close).toFixed(2) : '—';
        const chg = p ? parseFloat(p.change_pct) : null;
        const chgStr = chg === null ? '—' : (chg >= 0 ? '+' : '') + chg.toFixed(2) + '%';
        const cls = chg === null ? '' : chg >= 0 ? 'green' : 'red';
        return `<tr>
          <td style="font-weight:600;font-size:12px">${symbol.replace('.SR','')}</td>
          <td style="font-size:12px;color:#ccc">${name}</td>
          <td style="font-size:12px;text-align:right">${close}</td>
          <td style="font-size:12px;text-align:right" class="${cls}">${chgStr}</td>
        </tr>`;
      }).join('');

      container.innerHTML = `<table style="width:100%;border-collapse:collapse">
        <thead><tr>
          <th style="font-size:11px;color:#666;text-align:left;padding:6px 4px">Symbol</th>
          <th style="font-size:11px;color:#666;text-align:left;padding:6px 4px">Name</th>
          <th style="font-size:11px;color:#666;text-align:right;padding:6px 4px">Close (SAR)</th>
          <th style="font-size:11px;color:#666;text-align:right;padding:6px 4px">Chg%</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>`;
    })
    .catch(() => {
      container.innerHTML = '<p style="color:#888;padding:16px;font-size:13px;">Price data unavailable.</p>';
    });
})();
