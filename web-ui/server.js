...
// EGX trades Sun–Thu. Fri (UTC day 5) and Sat (UTC day 6) are non-trading.
// Count hours between two timestamps that fall inside EGX trading days only,
...
function marketAdjustedAgeHours(value) {
  ...
  let adjusted = raw;
  // Walk back day-by-day from now to fromMs, subtracting non-trading hours.
  const cursor = new Date(now);
  while (cursor.getTime() > fromMs) {
    const dow = cursor.getUTCDay(); // 0=Sun ... 5=Fri, 6=Sat
    // Start of this UTC day
    ...
    // Move cursor to end of previous day
    ...
    if (currentDate) {
      ...
      const currentRows = await dbAllAsync(
        `SELECT symbol, prediction_date, final_signal, conviction, confidence,
         ...
         WHERE prediction_date = ${datePh} AND UPPER(symbol) NOT LIKE '%.SR'`,
        [currentDate]
      ).catch(err => isTableMissing(err) ? [] : Promise.reject(err));
...
app.get('/api/intelligence/quality', optionalAuth, async (req, res) => {
  try {
    const freshness = {};
    // `marketAware: true` sources only publish on EGX trading days (Sun–Thu),
    // so we skip Fri/Sat hours when computing staleness.
    const sources = [
...
app.get('/api/consensus', (req, res) => {
  const query = DATABASE_URL
    ? `SELECT DISTINCT ON (symbol)
       ...
       FROM consensus_results
       WHERE UPPER(symbol) NOT LIKE '%.SR'
